...
if (!is_file($ini)) {
	throw new \coro\Fio\Exceptions\InvalidArgument('File not found: ' . $ini);
}

$accounts = parse_ini_file($ini, true);
if ($accounts === false) {
	throw new \coro\Fio\Exceptions\InvalidState('You have bad format for ini file. Let\'s see account.example.ini.');
}

$fioFactory = new \coro\Fio\Utils\FioFactory($accounts);
$fioRead = $fioFactory->createFioRead();

foreach ($fioRead->movements('-1 month') as $transaction) {
	/* @var $transaction \coro\Fio\Response\Read\Transaction */
